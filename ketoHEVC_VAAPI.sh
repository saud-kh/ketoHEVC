...
for file in "$input_dir"/**/*.{mkv,mp4}; do
  ...
  map_arg=""
  codec_arg="-c:v hevc_vaapi -b:v 0 -c:a aac -b:a 160k"
  sub_arg=""
  ...
  ffmpeg -hwaccel vaapi -hwaccel_device /dev/dri/renderD128 -hwaccel_output_format vaapi -i "$file" $map_arg $sub_arg $codec_arg "$new_file"
done
